...
while ishandle(btn)
    ...
    h1 = plot(time, 20*output_data(:,1), 'Color', colors{1});
    ...
    elseif waveform(input_count) == 2
        current_data = captured_data*20;
        h3 = plot(time,current_data, 'Color', colors{input_count});
        ylabel('Amps into solenoid')
        xlabel('time (s)');
        plotmax = 20*amp*1.5; plotmin = -20*amp*1.5;
        ylim([plotmin, plotmax])
    ...
    elseif waveform(input_count) == 3
        current_data = captured_data;
        h4 = plot(time,current_data, 'Color', colors{input_count});
