...
%   legendtext{end+1} = char(['pickup coil']); addAnalogInputChannel(s,d(1).ID, 3, 'Voltage');  pickupcoil =1;
  ...
  legendtext{end+1} = char(['output voltage']);addAnalogInputChannel(s,d(1).ID, 0, 'Voltage');
...
text(time(crop_start),20*amp*1.5-1, ['peak-peak = ', num2str(20*(max(captured_data(:,2)-min(captured_data(:,2))))),' A']);  
