...
sim_sRate = 2.5e5;
amp = 1;
duration = .001; %1 second acquisition
...
freq_space = 500;
drive_freq_vector = (1e3:freq_space:125e3);
%  drive_freq_vector = 50e3*ones(size(drive_freq_vector));
...
%     
   ...
   if crop_end > numel(captured_data)
           subplot(2,2,1);plot(time(crop_start:end),1*captured_data(crop_start:end));
   else
    subplot(2,2,1);plot(time(crop_start:crop_end),1*captured_data(crop_start:crop_end));
   end
   % ylim([-1.2*20*amp, 1.2*20*amp]);
    ylabel('Amps');
    xlabel('time (s)');
   ...
   title(['Acquired signal (duration = ',num2str(duration),'s, freq = ',num2str(drive_freq),'Hz, Amp = ',num2str(amp),'V)']);
    ...
    rms_response(fcount) = sqrt(mean((1*captured_data).^2));
    amp_response(fcount) = (max(captured_data)-min(captured_data));
    
    subplot(2,2,3); plot((drive_freq_vector(1):freq_space:drive_freq),     amp_response/2);
    xlim([drive_freq_vector(1),drive_freq_vector(end)]);
    ylabel('Current Amplitude from Techron monitor');
    xlabel('drive frequency (HZ)');
    
     subplot(2,2,4); 

    semilogx((drive_freq_vector(1):freq_space:drive_freq), 20*log10(amp_response/(2*amp)));
    xlim([drive_freq_vector(1),drive_freq_vector(end)]);
    ylabel('gain from input to current monitor (dB)');
    xlabel('drive frequency (HZ)');
...
% hold on; 
% 
% loglog(drive_freq_vector, rms_response/(amp*sqrt(2)/2),'b', 'linewidth', 2);
