...
freq_space = 500;
drive_freq_vector = (1e3:freq_space:75e3);
...
%     
   ...
   if crop_end > numel(captured_data)
           subplot(2,2,1);plot(time(crop_start:end),20*captured_data(crop_start:end));
   else
    subplot(2,2,1);plot(time(crop_start:crop_end),20*captured_data(crop_start:crop_end));
   end
   ...
   title(['Acquired signal (duration = ',num2str(duration),'s, freq = ',num2str(drive_freq),'Hz, Amp = ',num2str(amp),'V)']);
    ...
    rms_response(fcount) = sqrt(mean((20*captured_data).^2));
    amp_response(fcount) = (max(captured_data)-min(captured_data));
    
    subplot(2,2,3); plot((drive_freq_vector(1):freq_space:drive_freq),     20*amp_response/2);
