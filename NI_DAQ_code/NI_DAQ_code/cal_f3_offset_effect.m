...
offset_amp_vec = [0.00005:0.0001:0.002];  offset_phase_vec = [-pi/2:pi/20:pi/2];
...
for aa = 1:A
 ...
 [f3_amp, f3_phase, data_raw] = f3_test_run([offset_amp_vec(aa),offset_phase(aa)],300,1,4, 0);
...
phase_unwrapped(pp,:) = unwrap(f3_phase_unwrapped0(pp,:));
end
...
title('f3 cal amp and phase sweep, preamp gain 500, HPF in');  legend(amp_legend{2:end}); xlim([-pi/2,pi/2]);


subplot(2,2,3); plot(offset_phase_vec, f3_amp_all_all);  xlim([-pi/2, pi/2]);   xlabel('offset phase (rad)'); ylabel('resulting f3 amp (mV)');
title('f3 cal amp and phase sweep, preamp gain 500, HPF in');  legend(amp_legend); xlim([-pi/2,pi/2]);


subplot(2,2,4); plot(1000*offset_amp_vec(2:end), phase_unwrapped(2:end,:));   xlabel('offset amps (mV)');  ylabel('resulting f3 phase (rad)'); title('f3 cal amp sweep, preamp gain 500, HPF in');
legend(phase_legend);
